...
namespace NGG {
    ...
    private:
        static void genMargin(FILE* file, size_t margin) {
            for (size_t i = 0; i < margin; i++)
                fprintf(file, "\t");
        }

        static void d_Linker(FILE* file, ASTNode *pNode, size_t margin) {
            processFurther(file, pNode->getLeft(), margin);
            processFurther(file, pNode->getRight(), margin);
        }

        static void d_FuncDecl(FILE* file, ASTNode *pNode, size_t margin) {
            ...
            fprintf(file, "\n\n%s %s %s", alias.alias, pNode->getLexeme().getString()->begin(), aliasLPA.alias);
        ...
        static void d_Identifier(FILE* file,ASTNode *pNode, size_t margin) {
            genMargin(file, margin);
            fprintf(file, "%s", pNode->getLexeme().getString()->begin());
        }

        static void d_Number(FILE* file,ASTNode *pNode, size_t margin) {
            genMargin(file, margin);
            fprintf(file, "%lg", pNode->getLexeme().getDouble());
        }

        static void d_AssignExpr(FILE* file,ASTNode *pNode, size_t margin) {
        ...
        static void d_VarDef(FILE* file,ASTNode *pNode, size_t margin) {
            auto alias = LexParser::findAlias(pNode->getLexeme().getType());
            auto aliasStEnd = LexParser::findAlias(Lex_StEnd);
            ...
            if (pNode->getRight() != nullptr && pNode->getRight()->getKind() != Kind_None) {
                fprintf(file, "%s ", aliasAssg.alias);
                processFurther(file, pNode->getRight(), 0);
            }
            fprintf(file, " %s\n", aliasStEnd.alias);
        }

        static void d_MaUnOperator(FILE* file,ASTNode *pNode, size_t margin) {
            auto aliasLPA = LexParser::findAlias(Lex_LPA);
            auto aliasRPA = LexParser::findAlias(Lex_RPA);
        ...
        static void d_MaOperator(FILE* file,ASTNode *pNode, size_t margin) {
            auto aliasLPA = LexParser::findAlias(Lex_LPA);
            auto aliasRPA = LexParser::findAlias(Lex_RPA);
            if (pNode->getLeft()->getKind() != Kind_Identifier && pNode->getLeft()->getKind() != Kind_Number)
                fprintf(file, "%s", aliasLPA.alias);
        ...
        static void d_FuncCall(FILE* file,ASTNode *pNode, size_t margin) {
            auto aliasLPA = LexParser::findAlias(Lex_LPA);
            auto aliasRPA = LexParser::findAlias(Lex_RPA);
            auto aliasComma = LexParser::findAlias(Lex_Comma);

            genMargin(file, margin);
            fprintf(file, "%s%s", pNode->getLexeme().getString()->begin(), aliasLPA.alias);
        ...
        static void d_CmpOperator(FILE* file,ASTNode *pNode, size_t margin) {
            auto aliasLPA = LexParser::findAlias(Lex_LPA);
            auto aliasRPA = LexParser::findAlias(Lex_RPA);
            if (pNode->getLeft()->getKind() != Kind_Identifier && pNode->getLeft()->getKind() != Kind_Number)
                fprintf(file, "%s", aliasLPA.alias);
        ...
        static void d_ReturnStmt(FILE* file,ASTNode *pNode, size_t margin) {
        ...
        static void d_Print(FILE* file,ASTNode *pNode, size_t margin) {
        ...
        static void d_IfStmt(FILE* file,ASTNode *pNode, size_t margin) {
        ...
        static void d_Input(FILE* file,ASTNode *pNode, size_t margin) {
            auto alias = LexParser::findAlias(Lex_Input);
            fprintf(file, " %s ", alias.alias);
        }

        static void d_WhileStmt(FILE* file,ASTNode *pNode, size_t margin) {
        ...
        static void processFurther(FILE* file, ASTNode *head, size_t margin) {
            if (head == nullptr)
                return;
            switch (head->getKind()) {
                case Kind_Linker:
                    d_Linker(file, head, margin);
                    break;
                case Kind_FuncDecl:
                    d_FuncDecl(file, head, margin);
                    break;
                case Kind_Identifier:
                    d_Identifier(file, head, margin);
                    break;
                case Kind_Number:
                    d_Number(file, head, margin);
                    break;
                case Kind_AssignExpr:
                    d_AssignExpr(file, head, margin);
                    break;
                case Kind_VarDef:
                    d_VarDef(file, head, margin);
                    break;
                case Kind_MaUnOperator:
                    d_MaUnOperator(file, head, margin);
                    break;
                case Kind_MaOperator:
                    d_MaOperator(file, head, margin);
                    break;
                case Kind_FuncCall:
                    d_FuncCall(file, head, margin);
                    break;
                case Kind_CmpOperator:
                    d_CmpOperator(file, head, margin);
                    break;
                case Kind_ReturnStmt:
                    d_ReturnStmt(file, head, margin);
                    break;
                case Kind_Print:
                    d_Print(file, head, margin);
                    break;
                case Kind_IfStmt:
                    d_IfStmt(file, head, margin);
                    break;
                case Kind_Input:
                    d_Input(file, head, margin);
                    break;
                case Kind_None:
                    break;
                case Kind_WhileStmt:
                    d_WhileStmt(file, head, margin);
                    break;
                default: {
